...
/************************************ 修改部分 start ******************************************/
void System::PubImageData(double dStampSec, ImgConstPtr features)
{
    if (!init_feature)
    {
    ...
    if (PUB_THIS_FRAME)
    {
        ...
        for (int i = 0; i < NUM_OF_CAM; i++)
        {
            ...
            {
                m_buf.lock();
                feature_buf.push(features);
                // cout << "5 PubImage t : " << fixed << features->header
