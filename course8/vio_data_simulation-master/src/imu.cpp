...
void IMU::testImu(std::string src, std::string dist)
{
    ...
    /************************************ 添加部分 start ******************************************/
    // 添加变量
    Eigen::Vector3d acc_w_last = init_Rwb_ * (imudata[0].imu_acc) + gw;     //保存上一时刻世界坐标系下的运动加速度
    Eigen::Vector3d imu_gyro_last = imudata[0].imu_gyro;                    //保存上一时刻imu坐标系下的陀螺测量值
    /************************************ 添加部分 end ******************************************/
    ...
    for (int i = 1; i < imudata.size(); ++i) {
        ...
        //中值法计算更新四元数：delta_q = [1 , 1/2 * thetax , 1/2 * theta_y, 1/2 * theta_z]
        Eigen::Quaterniond dq;
        Eigen::Vector3d dtheta_half =  (imu_gyro_last + imupose.imu_gyro)/2.0 * dt /2.0;
        dq.w() = 1;
        dq.x() = dtheta_half.x();
        dq.y() = dtheta_half.y();
        dq.z() = dtheta_half.z();
        ...
        //计算当前时刻世界坐标系下的运动加速度 aw = Rwb * ( acc_body - acc_bias ) + gw
        Eigen::Vector3d acc_w = Qwb * (imupose.imu_acc) + gw;  

        //位置更新
        Pwb = Pwb + Vw * dt + 0.5 * dt * dt * (acc_w_last + acc_w)/2.0;

        //速度更新
        Vw = Vw + (acc_w_last + acc_w)/2.0 * dt;

        //保存上一时刻的值，用于下一时刻计算中值
        acc_w_last = acc_w;
        imu_gyro_last = imupose.imu_gyro;
        /************************************ 添加部分 end ******************************************/
